...
def ss2sim(sys, synapse, dt=None):
    ...
    if synapse.analog and len(num.coeffs) > 1:
        raise ValueError("analog synapse (%s) must not have zeros" % synapse)
    ...
    def __init__(
        ...
        **ens_kwargs
        ...
        with self:
            ...
            x_label = "%s:state" % ("" if self.label is None else self.label)
            ...
            if system.input_size == 0:
                ...
                self.ss_connections["B"] = nengo.Connection(
                    self.input, self.state_input, transform=B, synapse=self.synapse
                )

            self.output = nengo.Node(size_in=system.output_size)
            self.ss_connections["C"] = nengo.Connection(
                self.state_output, self.output, transform=C, synapse=output_synapse,
            )
